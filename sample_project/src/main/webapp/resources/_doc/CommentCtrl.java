...
public class CommentCtrl {
	...
	public ResponseEntity<String> remove(@PathVariable("cno")int cno){
		...
		return csv.remove(cno) > 0 ? new ResponseEntity<String>(cno + "댓글을 삭제하였습니다.", HttpStatus.OK) : new ResponseEntity<String>(HttpStatus.INTERNAL_SERVER_ERROR);
	}
	...
	public ResponseEntity<String> modify(@PathVariable("cno")int cno, @RequestBody CommentVO cvo){
		...
		cvo.setCno(cno);
		return csv.modify(cvo) == 1 ? new ResponseEntity<String>("댓글이 수정되었습니다", HttpStatus.OK) : new ResponseEntity<String>(HttpStatus.INTERNAL_SERVER_ERROR);
	}
